...
class VerkaufMitVerlustTest {
	...
	void testVerkauf_mit_Verlust() {
		...
		Buchung verkauf = generator.erzeugeHandel(BuchungArt.Verkauf, valueOf(-100.0), valueOf(-1000.0));
		impl.buchen(verkauf);
		...
		Konto kursVerlust = 
				konten.stream()
				.filter( k -> k.getName().equals(KURS_VERLUST) )
				.findFirst().get();
		assertTrue(valueOf(500.0).compareTo(kursVerlust.getSaldo()) == 0);
	}
}
